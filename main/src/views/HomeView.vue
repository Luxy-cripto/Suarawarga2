...
async function fetchReports(showLoading = true) {
  ...
  try {
     ...
     * jika sebelumnya sudah ada data.
     */
...
async function fetchKategoris(showLoading = true) {
  if (showLoading) {
  ...
  try {
    ...
    /*
     * Jangan menghilangkan data kategori
     * yang sudah tampil ketika auto-refresh gagal.
     */
    if (!kategoris.value.length) {
      kategoris.value = []
    }
  } finally {
    if (showLoading) {
...
async function refreshData() {
  /*
   * Saat refresh otomatis:
   * - Tidak menampilkan loading ulang
   * - Tidak membuat halaman berkedip
   * - Data langsung diperbarui jika ada perubahan
   */

  await Promise.all([
    fetchReports(false),
    fetchKategoris(false)
  ])
}
...
onMounted(async () => {
  ...
  /*
   * Load pertama kali dengan loading.
   */
  await Promise.all([
    fetchReports(true),
    fetchKategoris(true)
...
onUnmounted(() => {
  /*
   * Hentikan setInterval ketika user
