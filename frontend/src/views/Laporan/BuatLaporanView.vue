...
function handleFotoChange(event) {
  ...
  if (!allowedTypes.includes(file.type)) {
    errorMessage.value = 'Foto harus berformat JPG, JPEG, PNG, atau WEBP.'
    event.target.value = ''
    form.value.foto = null
    fotoPreview.value = ''
    return
  ...
  if (file.size > 2 * 1024 * 1024) {
    errorMessage.value = 'Ukuran foto maksimal 2 MB.'
    event.target.value = ''
    form.value.foto = null
    fotoPreview.value = ''
    return
...
function resetForm() {
  form.value = {
    ...
    lokasi: '', latitude: '', longitude: '', foto: null,
  }
  fotoPreview.value = ''
  errorMessage.value = ''
          ...
          <div class="form-group">
            ...
            <div class="upload-box">
              ...
              <div class="upload-content">
                <strong>Tambahkan foto bukti</strong>
                <small>Foto membantu petugas memahami kondisi masalah.</small>
                <input id="foto" type="file" accept="image/jpeg,image/png,image/jpg,image/webp" @change="handleFotoChange" />
              </div>
            </div>
            <small class="help-text">📸 Format JPG, JPEG, PNG, atau WEBP. Maksimal 2 MB.</small>
          </div>

          <div v-if="fotoPreview" class="preview-wrapper">
            <div class="preview-header">
              <strong>🖼️ Preview Foto</strong>
              <button type="button" class="remove-photo" @click="form.foto = null; fotoPreview = ''; document.getElementById('foto').value = ''">
                ✕ Hapus
              </button>
            </div>
            <img :src="fotoPreview" alt="Preview foto bukti" class="foto-preview" />
          </div>
...
@media (max-width: 600px) {
  ...
  .foto-preview { width: 100%; height: 190px; }
